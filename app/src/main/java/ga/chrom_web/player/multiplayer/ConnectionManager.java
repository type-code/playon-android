    ...
    protected void subscribeOnEvents() {
        socket.on(EVENT_CONNECTED, args -> {
            ...
            if (connectionListener != null) {
                connectionListener.connected(JsonUtil.INSTANCE.jsonToObject(args[0], PlayerData.class));
            }
        });
        socket.on(EVENT_JOIN, args -> {
            Utils.debugLog("Someone joined maybe it's me: " + args[0]);
            if (connectionListener != null) {
                connectionListener.joined(JsonUtil.INSTANCE.parseNick(args[0]));
            }
        });
        socket.on(EVENT_DISCONNECT, args -> {
            Utils.debugLog("Someone disconnect: " + args[0]);
            if (connectionListener != null) {
                connectionListener.someoneDisconnected(JsonUtil.INSTANCE.parseNick(args[0]));
            }
        });
